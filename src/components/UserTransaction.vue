      ...
      if(this.value < 0)
      {
        ...
        .catch((error) => {
          if (error.response.status == "400")
            self.mensajeError = "El valor de la transacción debe ser positivo"
          
          else if (error.response.status == "422")
            self.mensajeError = "El valor de la transacción debe ser numérico"
          
          else       
            self.mensajeError = "Saldo Insuficiente"
        });
      }
